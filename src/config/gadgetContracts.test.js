...
describe('BAR gadget contract registry', () => {
  it('is versioned and pinned to the bundled BAR snapshot', () => {
    expect(GADGET_CONTRACT_REGISTRY_VERSION).toBe(1);
    expect(GADGET_CONTRACT_REGISTRY.length).toBeGreaterThanOrEqual(6);
  ...
  it('keeps contract identifiers and runtime sources unique', () => {
    expect(new Set(GADGET_CONTRACT_REGISTRY.map(contract => contract.id)).size)
      .toBe(GADGET_CONTRACT_REGISTRY.length);
    expect(new Set(GADGET_CONTRACT_REGISTRY.map(contract => contract.source.path)).size)
      .toBe(GADGET_CONTRACT_REGISTRY.length);
  });
});
